...
def get_data_from_csv():
    try:
        df = pd.read_csv("COVIDdata16Final2022.csv")
    except:
...
da.rename(
    columns={"New_cases": "Total Cases", "New_deaths": "Total Deaths"}, inplace=True
)
...
if total_covid19_cases and total_covid19_deaths != 0:
    ...
    with right_column:
        ...
        if total_covid19_p != 0:
            ...
            st.subheader(f"0")
        with right_column2:
            st.subheader("Last Updated For 2022")
            st.subheader(f"4-05-2023")

        # Total Covid-19 Cases by Month [BarChart]

        totalcovid19_cases_by_month = (
            df_selection.groupby(by="Month")
            .sum()[["Total Cases"]]
            .sort_values(by="Month")
        )

        totalcovid19_cases_by_month.rename(
            index={
                1: "January",
                2: "February",
                3: "March",
        ...
        fig_month_total = px.bar(
            totalcovid19_cases_by_month,
            x=totalcovid19_cases_by_month.index,
            y="Total Cases",
            title="<b>Total Covid Cases By Month</b>",
            color_discrete_sequence=["#95A5A6"] * len(totalcovid19_cases_by_month),
            text_auto=".2s",
            template="plotly_white",
        )

        fig_month_total.update_traces(
            textfont_size=12, textangle=0, textposition="outside", cliponaxis=False
        )
        fig_month_total.update_layout(
            plot_bgcolor="rgb(205, 97, 85)",
            yaxis=(dict(showgrid=False)),
            xaxis=dict(tickmode="linear"),
        )

        # Total Covid-19 Cases by Month [Line Chart]
        fig_month_total_lineplot = px.line(
            totalcovid19_cases_by_month,
            x=totalcovid19_cases_by_month.index,
            y="Total Cases",
        ...
        fig_month_total_lineplot.update_layout(
            plot_bgcolor="rgb(205, 97, 85)",
            yaxis=(dict(showgrid=False)),
            xaxis=dict(tickmode="linear"),
        )
    ...
    Deaths_by_month = (
        df_selection.groupby(by="Month").sum()[["Total Deaths"]].sort_values(by="Month")
    )

    Deaths_by_month.rename(
        index={
            1: "January",
            2: "February",
            3: "March",
    ...
    fig_Monthly_Death = px.bar(
        Deaths_by_month,
        x=Deaths_by_month.index,
        y="Total Deaths",
        title="<b>Deaths By Month</b>",
        color_discrete_sequence=["#95A5A6"] * len(Deaths_by_month),
        template="plotly_white",
        text_auto=".2s",
    )
    fig_Monthly_Death.update_traces(
        textfont_size=12, textangle=0, textposition="outside", cliponaxis=False
    )
    fig_Monthly_Death.update_layout(
        xaxis=dict(tickmode="linear"),
        plot_bgcolor="rgb(205, 97, 85)",
        yaxis=(dict(showgrid=False)),
    )
